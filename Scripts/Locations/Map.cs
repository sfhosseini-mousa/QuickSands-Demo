...
namespace Sands
{
    ...
    public class Map : MonoBehaviour
    {
        ...
        [SerializeField] GameObject questFailPopUp;
        [SerializeField] Text questFailText;
        ...
        public void goBtnClicked()
        {
            ...
            {
                if(quest.QuestName == "Transport" && !quest.Completed)
                {
                    if(clickedlocation.LocationName != quest.QuestLocation.LocationName)
                    {
                        questFailPopUp.SetActive(true);
                        questFailText.text = "The selected Town is different from Transport Quest destination. Hero " + ((TransportQuest)quest).QuestHero.GetType().Name + "will leave and the quest will fail.";
                        goThrough = false;
                    }
                }
            }
        ...
        public void AcceptQuestFailOnClick()
        {

            for (int i = 0; i < Player.AcceptedQuests.Count; i++)
            {
                if (Player.AcceptedQuests[i].QuestName == "Transport" && !Player.AcceptedQuests[i].Completed && Player.AcceptedQuests[i].QuestLocation.LocationName != clickedlocation.LocationName) {

                    ((TransportQuest)Player.AcceptedQuests[i]).RemoveQuestHero();

                    Player.AcceptedQuests.RemoveAt(i);
                    i--;
                }
            }
            GoToLocation();
        ...
        void CheckTransportQuests()
        {
            foreach (var quest in Player.AcceptedQuests)
            {
                if (quest.QuestName == "Transport" && !((TransportQuest)quest).HeroJoinChecked && quest.QuestLocation.LocationName == Player.CurrentLocation.LocationName)
                    ((TransportQuest)quest).HeroJoinChecked = true;
            }
        }
